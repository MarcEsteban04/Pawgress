...
export function FocusShell({
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-surface-sunken">
      <header className="flex h-14 shrink-0 items-center gap-4 border-b border-rule bg-paper px-4 sm:px-6">
        <Link
          href={exitHref}
        ...
        {meta && <span className="hidden font-mono text-xs text-ink-subtle sm:block">{meta}</span>}
      </header>

      {progress !== undefined && (
        <div
          className="h-1 shrink-0 bg-rule"
          role="progressbar"
          aria-valuemin={0}
          aria-valuemax={100}
        ...
        >
          <div
            className="h-full bg-ink-muted transition-[width]"
