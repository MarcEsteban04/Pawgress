...
| 55 | Study tracking — sessions recorded, `/progress` built, dashboard fed | done | **out of order, on request.** `progress`, `study_sessions` and `quiz_attempts` had existed since Sprint 13 and the dashboard read all three since Sprint 23 — nothing ever wrote to them. Counted figures only; the weighted mastery formula is still Sprint 56 |
